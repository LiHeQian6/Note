$(document).ready(function() {
    ...
    $("#addRow").on("click",
    function() {
        t.row.add([counter + ".1", counter + ".2", counter + ".3", counter + ".4", counter + ".5"]).draw(!1),
        counter++
    }),
    ...
    $(".datatable-select-inputs").DataTable({
        initComplete: function() {
            this.api().columns().every(function() {
                var column = this,
                select = $('<select><option value="">Select option</option></select>').appendTo($(column.footer()).empty()).on("change",
                function() {
    ...
    $(".show-child-rows tbody").on("click", "td.details-control",
    function() {
        var tr = $(this).closest("tr"),
        row = tableChildRows.row(tr);
        row.child.isShown() ? (row.child.hide(), tr.removeClass("shown")) : (row.child(format(row.data())).show(), tr.addClass("shown"))
    });
    var multipleRowsTable = $(".selection-multiple-rows").DataTable();
    $(".selection-multiple-rows tbody").on("click", "tr",
    function() {
    ...
    $(".selection-deletion-row tbody").on("click", "tr",
    function() {
        $(this).hasClass("selected") ? $(this).removeClass("selected") : (tableselectionDelete.$("tr.selected").removeClass("selected"), $(this).addClass("selected"))
    }),
    $("#delete-row").on("click",
    function() {
    ...
    $("input.global_filter_search").on("keyup click",
    function() {
        filterGlobal()
    }),
    $("input.column_filter_search").on("keyup click",
    function() {
