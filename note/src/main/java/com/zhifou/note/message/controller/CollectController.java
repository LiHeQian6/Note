...
public class CollectController {
    ...
    @PostMapping("/collect")
    public void collect(@RequestParam int id){
    ...
    @GetMapping("/collect")
    public List<Map<String, Object>> getCollects(@ApiParam("从第x(first:0)个开始")@RequestParam int offset,
                                                 @ApiParam("每页有几个")@RequestParam  int limit) throws NoteException {
        User userInfo = jwtUtils.getUserInfo();
        return collectService.getCollects(userInfo.getId(),offset,limit);
    }
    @ApiOperation("获取收藏该笔记的用户列表")
    @GetMapping("/collector/{noteId}")
    public List<Map<String, Object>> getFollowers(@ApiParam("从第x(first:0)个开始")@RequestParam int offset, @ApiParam("每页有几个")@RequestParam int limit, @PathVariable int noteId) throws NoteException {
        User userInfo = jwtUtils.getUserInfo();
