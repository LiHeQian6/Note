...
class LikeNoticeFragment : Fragment() {
        ...
        handler = object: Handler(){
            override fun handleMessage(msg: Message) {
                ...
                if(result != null && result.get("status") == "SUCCESS"){
                    ...
                    if (newList.size == 0) {
                        like_notice_refresh.finishLoadMore()
    ...
    private fun initRefreshLayout() {
        like_notice_refresh.setEnableLoadMore(true)
        like_notice_refresh.autoRefresh()
        like_notice_refresh.setOnRefreshListener {
            currentPage = 0
            initData()
        }
        like_notice_refresh.setOnLoadMoreListener() {
            currentPage++
            initData()
            if (newList.size == 0)
    ...
    private fun initList() {
        ...
        adapter = OptionNoticeAdapter(list,requireContext(),like_recyclerView)
    ...
    private fun initData() {
        ...
        urlBuilder.addPathSegment("like")
