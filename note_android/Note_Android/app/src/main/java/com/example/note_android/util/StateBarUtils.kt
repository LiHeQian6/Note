...
class StateBarUtils {
    ...
    companion object{
        ...
        fun supportTranslucent(): Boolean {
            return (Build.VERSION.SDK_INT >= VERSION_CODES.KITKAT // Essential Phone 在 Android 8 之前沉浸式做得不全，系统不从状态栏顶部开始布局却会下发 WindowInsets
        ...
        fun translucent(window: Window, @ColorInt colorOn5x: Int) {
            ...
            if (DeviceUtils.isMeizu() || DeviceUtils.isMIUI() && Build.VERSION.SDK_INT < VERSION_CODES.M) {
                window.setFlags(
...
//            }
         ...
         */
        fun initStatusBarStyle(activity: Activity, isDark: Boolean) {
        ...
        private fun changeStatusBarModeRetainFlag(window: Window, out: Int): Int {
            ...
            out = retainSystemUiFlag(window, out, View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN)
            out =
                retainSystemUiFlag(window, out, View.SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION)
            return out
        }
        ...
        fun customFullScreen(activity: Activity) {
            if (Build.VERSION.SDK_INT >= VERSION_CODES.KITKAT) {
                if (Build.VERSION.SDK_INT >= VERSION_CODES.LOLLIPOP) {
